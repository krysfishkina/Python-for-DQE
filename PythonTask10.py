...
class News(Publication):
    def __init__(self):
        super().__init__()
        self.city = normalize_text(input('Please enter the city:\n'))
        self.news_date = f"{self.city}, {date.now().strftime('%d/%m/%Y %I.%M')}"
        ...
        with pyodbc.connect('DRIVER={SQLite3 ODBC Driver};'
            ...
            with self.connection.cursor() as self.cursor:
                ...
                self.cursor.execute('CREATE TABLE IF NOT EXISTS ads '
                                    '(text text, actual_date text, count_days_until text, PRIMARY KEY(text,actual_date))')
                self.cursor.execute('CREATE TABLE IF NOT EXISTS weather '
                                    '(text text, actual_date text, '
                                    'country text, city text, temperature text,'
                                    'until_spring text, PRIMARY KEY(text,actual_date,country,city))')
        ...
        if type_of_publication.lower() == 'news':
            try:
                ...
                if unique in er_code:
                    print('duplicate row')
        elif type_of_publication.lower() == 'ads':
            try:
                self.cursor.execute(f"INSERT INTO ads VALUES "
                                    f"('{kwargs['text']}','{kwargs['actual_date']}','{kwargs['count_days_until']}')")
            except pyodbc.Error as error:
                unique = "('HY000'"
                er_code = ''.join(str(error)).split(',')
                if unique in er_code:
                    print('duplicate row')
        elif type_of_publication.lower() == 'weather forecast':
            try:
                self.cursor.execute(f"INSERT INTO weather VALUES "
                                    f"('{kwargs['text']}','{kwargs['actual_date']}','{kwargs['country']}', '{kwargs['city']},'{kwargs['temperature']}, '{kwargs['until_spring']})")
            except pyodbc.Error as error:
                unique = "('HY000'"
                er_code = ''.join(str(error)).split(',')
                if unique in er_code:
        ...
        for index, val in enumerate(list_for_db):
            if val.lower() == 'news':
                ...
                temp_dict = {'type': val.lower(),
                             ...
                             'count_days_until': list_for_db[index + 3]}
                self.list_of_dicts_for_db.append(temp_dict.copy())
                temp_dict.clear()
            elif val.lower() == 'weather forecast':
                temp_dict = {'type': val.lower(),
                             'text': list_for_db[index + 1],
                             'actual_date': list_for_db[index + 2],
                             'country': list_for_db[index + 3],
                             'city': list_for_db[index + 4],
                             'temperature': list_for_db[index + 5],
                             'until_spring': list_for_db[index + 6]}
                ...
                if key.lower() == 'type' and value.lower() == 'news':
                    ...
                    try:
                        self.cursor.execute(f"INSERT INTO ads VALUES ('{publ_dict['text']}',"
                                            f"'{publ_dict['actual_date']}','{publ_dict['count_days_until']}')")
                    except pyodbc.Error as error:
                        unique = "('HY000'"
                        er_code = ''.join(str(error)).split(',')
                        if unique in er_code:
                    ...
                    try:
                        self.cursor.execute(f"INSERT INTO weather VALUES ('{publ_dict['text']}',"
                                            f"'{publ_dict['actual_date']}','{publ_dict['country']}','{publ_dict['city']}','{publ_dict['city']},'{publ_dict['temperature']},'{publ_dict['until_spring']})")
                    except pyodbc.Error as error:
                        unique = "('HY000'"
                        er_code = ''.join(str(error)).split(',')
                        if unique in er_code:
...
while True:
    ...
    if select_type.input_type == '1':
        if select_type.type_of_publication == '1':
            ...
            db = Connection()
            db.input_insert_in_db('ads', text=new_ad.text_of_publication, actual_date=new_ad.ad_actual_date,
                                  count_days_until=new_ad.count_days_until)
            select_type.words_count_csv_write()
            select_type.characteristics_write()
        elif select_type.type_of_publication == '3':
            new_wf = WeatherForecastWithWishes()
            new_wf .write_to_file()
            db = Connection()
            db.input_insert_in_db('weather forecast', text=new_wf.text_of_publication, actual_date=new_wf.actual_date,
                                  country=new_wf.country, city=new_wf.city, temperature=new_wf.temperature, until_spring=new_wf.until_spring)
            select_type.words_count_csv_write()
            select_type.characteristics_write()
